...
| **Итого** | 100% | **441 388** | [\[1\]](#источники-3-1) — раздел «Население по федеральным округам» |
...
**1.** [Росстат — Население по федеральным округам](https://rosstat.gov.ru/bgd/regl/b21_14p/Main.htm) (раздел «Население по федеральным округам»; суммируй ФО и вычисли доли) — трафик по ДЦ 40/25/20/15%
...
**Откуда взяты имена таблиц:**

| Таблица | Паттерн | Источник |
|---------|---------|----------|
| `chats`, `messages`, `chat_members` | Cloud Chats: история на сервере + состав участников | [\[5\]](#источники-5-5) — Cloud vs Secret Chats |
| `user_updates` | Fan-out: персональная очередь событий на каждого получателя | [\[6\]](#источники-5-6) — Iris Queue (Meta Messenger) |
| `device_sync_state` | Delta Sync: курсор «с какого seq_no догонять» | [\[3\]](#источники-5-3) — `pts`, `getDifference` |
| `users`, `user_sessions` | Аккаунт + multi-device сессии | [\[2\]](#источники-5-2) — chat system design |
| `user_presence` | Online/offline вне основной БД | [\[2\]](#источники-5-2) — presence в Redis |
| `media_files` | Метаданные медиа, дедупликация по hash | [\[4\]](#источники-5-4) — deduplication (Meta) |
...
Обновляется каждые 25 секунд. Нагрузка: 52 083 avg RPS только на чтение (каждая доставка сообщения проверяет «онлайн ли получатель?»). Хранится только в Redis — in-memory, 0.1 мс latency, TTL встроен. ([\[2\]](#источники-5-2) — Ctrl+F: *"presence"*: почему presence в Redis, не в SQL)
...
**5.** [core.telegram.org/techfaq](https://core.telegram.org/techfaq) (Ctrl+F: *"Cloud chats"*, *"Secret chats"*) — паттерн Cloud Chats: `chats`, `messages`, `chat_members`
...
Источник: [\[1\]](#источники-6-1) — Ctrl+F: *"colocation"*: `messages` и `chat_members` с одним `chat_id` хранятся на одном воркере.
...
52 083 avg RPS на чтение presence. PostgreSQL потребовал бы 10–15 серверов и давал ~5 мс latency. Redis делает это на одном сервере с 0.1 мс. TTL встроен: `SET presence:user123 "online" EX 60` — через 60 сек запись исчезает автоматически. ([\[2\]](#источники-6-2) — Ctrl+F: *"16384 hash slots"*)
