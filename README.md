...
| **Доля медиа-вложений** | **14 %** | *Допущение, обоснованное публичными данными WhatsApp (2020):* 4.5 млрд фото + 1 млрд видео в день при ~40 млрд отправленных сообщений = ~14%. Для MAX принимается аналогичный показатель как для сопоставимого по аудитории мессенджера [[2]](#ref2)[[3]](#ref3). |
...
*   Размер сжатого медиафайла: **~5 МБ** (*Допущение, обоснованное взвешенным средним по типам медиа в 2026 году:* современные смартфоны снимают в 4K/HDR, фото в HEIC/WebP весят 2–4 МБ, короткие видео 15–60 сек в HD — 10–30 МБ [[6]](#ref6). Структура медиа-вложений: ~70% фото (~2 МБ), ~20% видео (~20 МБ), ~10% голосовые/документы (~1.5 МБ). Взвешенное среднее: (0.70 × 2) + (0.20 × 20) + (0.10 × 1.5) = 1.4 + 4.0 + 0.15 ≈ **5.5 МБ**, округляем до **5 МБ**).
...
| **Медиа (Object / CDN)** | (2 917 + 7 292) × 5 МБ | ~51 045 МБ/с | **~399.6 Gbit/s** | **~799.2 Gbit/s** |
| **ИТОГО Трафик** | Текст + Медиа | ~51 082 МБ/с | **~399.9 Gbit/s** | **~799.8 Gbit/s** |

> **Вывод:** Текстовый трафик генерирует высокую нагрузку на CPU серверов (парсинг JSON, маршрутизация миллионов соединений), но занимает ничтожно мало канала (~0.6 Гбит/с). Основной сетевой канал «съедают» медиафайлы — с долей медиа 14%, fan-out 2.5 и средним размером файла 5 МБ (2026 год: фото в HEIC/WebP, короткие HD-видео) требуется пропускная способность около **800 Gbit/s** на пиках. Это обуславливает обязательное использование CDN для медиа-трафика.
...
| **Медиа (S3/Object)** | 45 млн × 40 × 14% × 5 МБ = **~1 260 ТБ/сут.** | 1 260 × 365 = **~459 900 ТБ (~449 ПБ)** | **~1 347 ПБ (~1.35 ЭБ)** |

> **Что использовать:** Для ~1 Петабайта текстовых метаданных потребуется распределенная NoSQL база (например, Cassandra или ScyllaDB, как используют в Discord [[4]](#ref4)). Для медиафайлов (~1.35 Экзабайта за год) используется S3-совместимое хранилище (MinIO, Ceph или отказоустойчивые решения облачных провайдеров). При таком объёме критически важна политика хранения: автоматическое удаление/архивирование медиа старше N лет или введение лимитов на хранение для пользователей.
...
6. <a id="ref6"></a>**Сжатие видео для WhatsApp и Telegram:** Анализ ограничений и битрейтов мессенджеров. WhatsApp ограничивает видео до 16 МБ при битрейте 500–1000 kbps (480p), типичное видео 30–60 сек ≈ 2–4 МБ. URL: [litespace.io/blog/how-to-compress-video-for-whatsapp-and-telegram-without-quality-loss](https://www.litespace.io/blog/how-to-compress-video-for-whatsapp-and-telegram-without-quality-loss) *(Обоснование среднего размера медиафайла ~1 МБ через взвешенное среднее по типам вложений).*
...
**Обоснование долей.** Каждое открытие приложения порождает запросы сразу к нескольким доменам: `auth.` (валидация токена), `msg.` (список диалогов + WebSocket), `profile.` (аватары и имена собеседников), `search.` (поиск по списку чатов). Открытие конкретного чата дополнительно нагружает `profile.` (профили участников) и `msg.` (история). Отправка медиа — `media.`. Исходя из этого профиля сессии, доли распределены следующим образом [^3]:

| Домен (Сервис) | Доля трафика | Средний RPS | Пиковый RPS | Специфика нагрузки на балансировщики |
| :--- | :---: | ---: | ---: | :--- |
| **msg.** | 48% | 34 560 | 86 400 | Удержание миллионов долгих TCP/WebSocket соединений, мелкие IOPS. Доминирует, но уступает часть трафика profile и search при каждом открытии чата. |
| **media.** | 20% | 14 400 | 36 000 | «Тяжёлые» файлы. Требует максимальной пропускной способности сети (Bandwidth) и CDN. Доля ниже msg, т.к. медиа только в 14% сообщений. |
| **profile.** | 18% | 12 960 | 32 400 | Преимущественно чтение (read-heavy). Загружается при каждом открытии чата (аватары, имена, статус online/offline участников). Значительно выше, чем кажется интуитивно. |
| **search.** | 10% | 7 200 | 18 000 | Поиск по списку диалогов происходит при каждом открытии приложения. Сложные текстовые запросы, высокая утилизация CPU (CPU-bound). |
| **auth.** | 4% | 2 880 | 7 200 | Валидация токена при каждом reconnect и запуске приложения. Редкие, но критичные операции (login, logout, refresh). |
...
3. **Локальная балансировка:** Полученные цифры RPS применяются для каждого ДЦ локально. Например, на `search.max.ru` в Москве в пике приходит 7 200 RPS (72 000 × 0.10). Это позволяет балансировщику внутри ДЦ направлять трафик на ограниченную группу мощных серверов, экономя бюджет.
...
- Суммарный пиковый трафик: **~799.8 Gbit/s**
...
| :--- | :---: | ---: | ---: | ---: | ---: | ---: | ---: |
| **DC1: Москва** | 40% | 66 500 | 320.0 | ⌈66 500 / 10 274⌉ = **7** | ⌈320.0 / 8.80⌉ = **37** | **37** | **38** |
| **DC2: Санкт-Петербург** | 25% | 41 563 | 200.0 | ⌈41 563 / 10 274⌉ = **5** | ⌈200.0 / 8.80⌉ = **23** | **23** | **24** |
| **DC3: Новосибирск** | 20% | 33 250 | 160.0 | ⌈33 250 / 10 274⌉ = **4** | ⌈160.0 / 8.80⌉ = **19** | **19** | **20** |
| **DC4: Ростов-на-Дону** | 15% | 24 938 | 120.0 | ⌈24 938 / 10 274⌉ = **3** | ⌈120.0 / 8.80⌉ = **14** | **14** | **15** |

### 4.3. Итоговое количество балансировщиков

| ДЦ | L4 (провайдер) | L7 рабочих | L7 резерв | L7 итого |
| :--- | :---: | :---: | :---: | :---: |
| DC1: Москва | 1 (VIP) | 37 | 1 | **38** |
| DC2: Санкт-Петербург | 1 (VIP) | 23 | 1 | **24** |
| DC3: Новосибирск | 1 (VIP) | 19 | 1 | **20** |
| DC4: Ростов-на-Дону | 1 (VIP) | 14 | 1 | **15** |
| **ВСЕГО** | **4** | **93** | **4** | **97** |
...
> **Вывод:** Наиболее нагруженный ДЦ (Москва) требует 38 L7-узлов (37 рабочих + 1 резерв). Суммарно по всем четырём ДЦ развёртывается **97 L7-балансировщиков**. Узким местом является пропускная способность сети (N_net >> N_ssl), что обусловлено высоким медиа-трафиком (~800 Gbit/s на пике при среднем размере медиафайла 5 МБ). Это подчёркивает критическую важность CDN для разгрузки балансировщиков от медиа-трафика.
